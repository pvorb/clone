...
/** Clones an Object using deep cloning.
	...
	@param `circular` - set to true if the object to be cloned may contain circular
		references. (optional - true by default)
*/
function clone(parent, circular) {
	...
	if(circular)
	{
			...
			if(typeof parent == "object")
			{
				...
				for(i in circularParent)
					if(circularParent[i] === parent)
					{
						...
						return null; //Just return null for now... we will resolve circular references later...
					}
		...
		//Now this object has been cloned. Let's check to see if there are any circular references for it
